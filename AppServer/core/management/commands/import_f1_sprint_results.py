# G:\Learning\3Tier_F1App\AppServer\core\management\commands\import_f1_sprint_results.py

import requests
from django.core.management.base import BaseCommand
from django.db import transaction, IntegrityError
from core.models import SprintResult, Race, Drivers, Constructors, Status
import json
from datetime import datetime


class Command(BaseCommand):
    help = 'Importa e atualiza dados de resultados de corridas sprint da API Ergast F1. Agora com paginação para trazer todos os registros.'

    def add_arguments(self, parser):
        parser.add_argument(
            '--year',
            type=int,
            help='Opcional. O ano para o qual importar/atualizar os dados de resultados de sprint. Padrão é o ano corrente.'
        )

    def handle(self, *args, **options):
        year = options['year'] if options['year'] is not None else datetime.now().year
        base_api_url = f"https://api.jolpi.ca/ergast/f1/{year}/sprint/"

        self.stdout.write(self.style.SUCCESS(
            f'Iniciando importação paginada de resultados de sprint para o ano {year} de {base_api_url}...'))
        ...
        while True:
            ...
            try:
                ...
                if total_records is None:
                    ...
                    self.stdout.write(self.style.SUCCESS(
                        f"  Total de resultados de sprint (aproximado) a importar: {total_records}"))
                ...
                self.stderr.write(self.style.ERROR(
                    f'Ocorreu um erro inesperado durante a paginação para {api_url}: {e}'))
                skipped_error_results_count += total_records if total_records is not None else 0
                break

        total_collected_results = sum(len(race.get('SprintResults', [])) for race in all_races_data)
        self.stdout.write(self.style.SUCCESS(
            f'\nIniciando processamento de {len(all_races_data)} corridas (contendo um total de {total_collected_results} resultados de sprint) coletadas...'))

        last_result_global = SprintResult.objects.order_by('-resultid').first()
        ...
        for race_json in all_races_data:
            ...
            try:
                ...
                self.stderr.write(self.style.ERROR(
                    f'Erro: Corrida (Ano: {api_season}, Rodada: {api_round}) não encontrada no banco de dados. Ignorando resultados de sprint desta corrida.'))
                if 'SprintResults' in race_json:
                    skipped_error_results_count += len(race_json['SprintResults'])
                continue

            if 'SprintResults' in race_json:
                for result_json in race_json['SprintResults']:
                    ...
                    try:
                        ...
                        self.stderr.write(self.style.ERROR(
                            f'Erro: Piloto (Ref: {driver_api_id}) não encontrado no banco de dados. Ignorando este resultado de sprint.'))
                    ...
                    try:
                        ...
                        self.stderr.write(self.style.ERROR(
                            f'Erro: Construtor (Ref: {constructor_api_id}) não encontrado no banco de dados. Ignorando este resultado de sprint.'))
                        skipped_error_results_count += 1
                        continue
                    ...
                    try:
                        ...
                        self.stderr.write(self.style.ERROR(
                            f'Erro: Status (Valor: "{status_api_status}") não encontrado no banco de dados. Ignorando este resultado de sprint. Certifique-se de importar status primeiro.'))
                        skipped_error_results_count += 1
                        continue
                    ...
                    points = int(result_json['points']) if 'points' in result_json else None
                    ...
                    try:
                        with transaction.atomic():
                            existing_result = SprintResult.objects.filter(
                            ...
                            if existing_result:
                                ...
                                self.stdout.write(self.style.WARNING(
                                    f'Atualizado Resultado Sprint: Piloto {driver_obj.surname} - Corrida {api_round}'))
                            else:
                                result_data = {
                                ...
                                SprintResult.objects.create(**result_data)
                                new_results_count += 1
                                self.stdout.write(self.style.SUCCESS(
                                    f'Criado NOVO Resultado Sprint: Piloto {driver_obj.surname} - Corrida {api_round} (ID: {next_result_id_counter})'))
                                next_result_id_counter += 1

                    except IntegrityError as e:
                        skipped_error_results_count += 1
                        self.stderr.write(self.style.ERROR(
                            f'Erro de Integridade ao processar Resultado Sprint (Piloto {driver_api_id}, Corrida {api_round}): {e}'))
                    except Exception as e:
                        skipped_error_results_count += 1
                        self.stderr.write(self.style.ERROR(
                            f'Erro inesperado ao processar resultado Sprint (Piloto {driver_api_id}, Corrida {api_round}): {e}'))
            else:
                self.stderr.write(self.style.ERROR(
                    'Estrutura JSON inesperada para resultados de sprint. Verifique o formato da API.'))
                skipped_error_results_count += 1

        self.stdout.write(self.style.SUCCESS('\n--- Resumo da Importação de Resultados de Sprint ---'))
        self.stdout.write(self.style.SUCCESS(f'Resultados de Sprint Novos Inseridos: {new_results_count}'))
        self.stdout.write(self.style.WARNING(f'Resultados de Sprint Atualizados: {updated_results_count}'))
        self.stdout.write(self.style.ERROR(f'Resultados de Sprint com Erros/Ignorados: {skipped_error_results_count}'))
        self.stdout.write(self.style.SUCCESS('Importação de resultados de sprint concluída!'))
